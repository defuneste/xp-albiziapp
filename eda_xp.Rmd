...
* Le samedi 29 juin 2019 de 10h30 à 12h00 et regroupait 7 participants. 125 arbres (relevés) ont été comptés entre les participants (figure : \@ref(fig:releveactivite)). Le nombre de relevés par participant varie de 10 à 40 arbres. 
...
Cette zone représente `r round(st_area(zone.shp)/10000, 2)` ha, comprend `r nrow(arbre_xp_zone.shp)` arbres soit `r round(nrow(arbre_xp_zone.shp)/round(st_area(zone.shp)/10000, 2),2)` arbres par hectare. 
...
Après réflexion, j'ai décidé de mettre faux au nom d'espèce des platanes. 
...
Le tableau \@ref(tab:test) indique pour chaque participant le nombre de relevés effectués (`nb_releve`), le nombre de relevés avec le nom commun, le genre et l'espèce de renseignés (`nb_commun`, `nb_genre` et `nb_espece`) et le nombre de ces relevés qui sont bon pour chaque catégorie botanique (ex : `genre_bon` pour le nombre de relevés ou le genre est bon). 
...
tableau_participants <- xp_bota.shp %>% 
  ...
  summarise(nb_releve = n(), # nombre de nouvelles observations par participants
            ...
            commun_bon = sum(bota_commun, na.rm = T), #nombre de non commun bon
            nb_genre = sum(!is.na(genus)), # nombre de genre renseignés
            genre_bon = sum(bota_genus, na.rm = T), # nombre de genre bon
            nb_espece = sum(!is.na(specie)), # nombre d'especes latin renseigné
            espece_bon = sum(bota_specie, na.rm = T) # nombre d'espèce de bon
            ) 
library(knitr)
kable(tableau_participants, caption = 'Informations botaniques par participants', align = c( "c","c","c","c","c", "c","c","c"))
```
